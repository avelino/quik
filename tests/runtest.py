...
class TemplateTestCase(TestCase):
    ...
    def test_parser_returns_input_when_there_is_nothing_to_substitute(self):
        template = quik.Template("<html></html>")
        self.assertEqual("<html></html>", template.render({}))

    def test_parser_substitutes_string_added_to_the_context(self):
        template = quik.Template("Hello @name")
        self.assertEqual("Hello Chris", template.render({"name": "Chris"}))

    def test_dollar_left_untouched(self):
        template = quik.Template("Hello $ ")
    ...
    def test_unmatched_name_does_not_get_substituted(self):
        template = quik.Template("Hello $name")
        self.assertEqual("Hello $name", template.render({}))
    ...
    def test_if_statement_considers_None_to_be_false(self):
        ...
        self.assertEqual('', template.render({}))
        self.assertEqual('', template.render({'some_value': None}))
    ...
    def test_single_line_comments_skipped(self):
        ...
        self.assertEqual("Stuff\nMore stuff", template.render({}))

    def test_multi_line_comments_skipped(self):
        template = quik.Template('Stuff#*\n more comments *#\n and more stuff')
        self.assertEqual("Stuff and more stuff", template.render({}))
    ...
    def test_curly_end(self):
        ...
        self.assertEqual("monkey", template.render({}))

    def test_too_many_end_clauses_trigger_error(self):
        template = quik.Template('#if (1)true!#end #end ')
        self.assertRaises(quik.TemplateSyntaxError, template.render, {})
...
\\@email''', template.render({}))
    ...
    def test_true_elseif_evaluated_when_if_is_false(self):
        ...
        self.assertEqual(' two ', template.render(locals()))

    def test_false_elseif_skipped_when_if_is_true(self):
        template = quik.Template('#if (@value1) one #elseif (@value2) two #end')
        value1, value2 = True, False
        self.assertEqual(' one ', template.render(locals()))
    ...
    def test_else_evaluated_when_if_and_elseif_are_false(self):
        ...
        self.assertEqual(' three ', template.render(locals()))
    ...
    def test_get_position_strings_in_syntax_error(self):
        try: quik.Template('#else whatever').render({})
        except quik.TemplateSyntaxError as e:
    ...
    def test_get_position_strings_in_syntax_error_when_newline_after_error(self):
        try: quik.Template('#else whatever\n').render({})
        except quik.TemplateSyntaxError as e:
            self.assertEqual(['#else whatever',
                               '^'], e.get_position_strings())
        else: self.fail('expected error')
    ...
    def test_compare_greater_than_operator(self):
        ...
        self.assertEqual('', template.render({'value': 0}))
        self.assertEqual('', template.render({'value': 1}))
        self.assertEqual('yes', template.render({'value': 2}))

    def test_compare_greater_than_or_equal_operator(self):
        template = quik.Template('#if ( @value >= 1 )yes#end')
        self.assertEqual('', template.render({'value': 0}))
        self.assertEqual('yes', template.render({'value': 1}))
        self.assertEqual('yes', template.render({'value': 2}))

    def test_compare_less_than_operator(self):
        template = quik.Template('#if ( @value < 1 )yes#end')
        self.assertEqual('yes', template.render({'value': 0}))
        self.assertEqual('', template.render({'value': 1}))
        self.assertEqual('', template.render({'value': 2}))
    ...
    def test_or_operator(self):
        ...
        self.assertEqual('', template.render({'value1': False, 'value2': False}))
        self.assertEqual('yes', template.render({'value1': True, 'value2': False}))
        self.assertEqual('yes', template.render({'value1': False, 'value2': True}))
    ...
    def test_or_operator_considers_not_None_values_true(self):
        ...
        template = quik.Template('#if ( @value1 || @value2 )yes#end')
        self.assertEqual('', template.render({'value1': None, 'value2': None}))
        self.assertEqual('yes', template.render({'value1': SomeClass(), 'value2': False}))
        self.assertEqual('yes', template.render({'value1': False, 'value2': SomeClass()}))

    def test_and_operator(self):
        template = quik.Template('#if ( @value1 && @value2 )yes#end')
        self.assertEqual('', template.render({'value1': False, 'value2': False}))
        self.assertEqual('', template.render({'value1': True, 'value2': False}))
        self.assertEqual('', template.render({'value1': False, 'value2': True}))
        self.assertEqual('yes', template.render({'value1': True, 'value2': True}))
    ...
    def test_and_operator_considers_not_None_values_true(self):
        ...
        template = quik.Template('#if ( @value1 && @value2 )yes#end')
        self.assertEqual('', template.render({'value1': None, 'value2': None}))
        self.assertEqual('yes', template.render({'value1': SomeClass(), 'value2': True}))
        self.assertEqual('yes', template.render({'value1': True, 'value2': SomeClass()}))

    def test_parenthesised_value(self):
        template = quik.Template('#if ( (@value1 == 1) && (@value2 == 2) )yes#end')
        self.assertEqual('', template.render({'value1': 0, 'value2': 1}))
        self.assertEqual('', template.render({'value1': 1, 'value2': 1}))
        self.assertEqual('', template.render({'value1': 0, 'value2': 2}))
        self.assertEqual('yes', template.render({'value1': 1, 'value2': 2}))
    ...
    def test_compound_condition(self):
        ...
        self.assertEqual('', template.render({'value': False}))
        self.assertEqual('yes', template.render({'value': True}))

    def test_logical_negation_operator(self):
        template = quik.Template('#if ( !@value )yes#end')
        self.assertEqual('yes', template.render({'value': False}))
        self.assertEqual('', template.render({'value': True}))

    def test_logical_negation_operator_yields_true_for_None(self):
        template = quik.Template('#if ( !@value )yes#end')
        self.assertEqual('yes', template.render({'value': None}))
    ...
    def test_compound_binary_and_unary_operators(self):
        ...
        self.assertEqual('', template.render({'value1': False, 'value2': True}))
        self.assertEqual('', template.render({'value1': True, 'value2': False}))
        self.assertEqual('', template.render({'value1': True, 'value2': True}))
        self.assertEqual('yes', template.render({'value1': False, 'value2': False}))
    ...
    def test_cannot_redefine_macro(self):
        template = quik.Template('#macro hello:hi#end#macro hello:again#end')
        self.assertRaises(Exception, template.render, {})

    def test_include_directive_gives_error_if_no_loader_provided(self):
        template = quik.Template('#include ("foo.tmpl")')
        self.assertRaises(quik.TemplateError, template.render, {})
    ...
    def test_include_directive_yields_loader_error_if_included_content_not_found(self):
        class BrokenLoader:
            def load_text(self, name):
                raise IOError(name)
        template = quik.Template('#include ("foo.tmpl")')
        self.assertRaises(IOError, template.render, {}, loader=BrokenLoader())
    ...
    def test_parse_directive_gives_error_if_no_loader_provided(self):
        template = quik.Template('#parse ("foo.tmpl")')
        self.assertRaises(quik.TemplateError, template.render, {})
    ...
    def test_when_dictionary_does_not_contain_referenced_attribute_no_substitution_occurs(self):
        ...
        self.assertEqual(" @user.name ", template.render({'user':self}))
    ...
    def test_color_spec(self):
        template = quik.Template('<span style="color: #13ff93">')
        self.assertEqual('<span style="color: #13ff93">', template.render({}))
    ...
    def test_stop_directive(self):
        template = quik.Template("hello #stop world")
        self.assertEqual('hello ', template.render({}))
